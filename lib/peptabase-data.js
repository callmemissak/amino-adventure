...
function buildPlaceholder(entry) {
  ...
  return {
    ...
    mechanismSummary: "Mechanism summary pending verified researcher content.",
    primaryReceptor: "Pending",
    biologicalPathway: "Pending",
    clinicalTrialsCount: null,
    animalStudiesCount: null,
    mechanisticStudiesCount: null,
    evidenceLevel: "Limited",
    firstDiscoveredYear: "",
    commonResearchGoals: [],
    stackSynergies: [],
    knownLimitations: "Known limitations pending verified researcher content.",
    safetyNotes: "Safety notes pending verified researcher content.",
    reviewedBy: "",
    reviewDate: "",
    revisionNotes: "",
    citationCount: 0,
...
export function normalizeDbPeptide(record) {
  ...
  const references = parseArray(record.pubmed_links).map((url) => {
    const pmid = extractPmid(url) || String(url).split("/").filter(Boolean).pop();
    return {
      id: pmid,
      pmid,
  ...
  return {
    ...
    mechanismSummary: sentenceFrom(record.mechanism_of_action) || "Mechanism summary pending verified researcher content.",
    primaryReceptor: "Pending",
    biologicalPathway: "Pending",
    clinicalTrialsCount: null,
    animalStudiesCount: null,
    mechanisticStudiesCount: null,
    evidenceLevel: references.length >= 3 ? "Moderate" : references.length >= 1 ? "Early" : "Limited",
    firstDiscoveredYear: "",
    commonResearchGoals: applications,
    stackSynergies: [],
    knownLimitations: "Known limitations pending verified researcher content.",
    safetyNotes: "Safety notes pending verified researcher content.",
    reviewedBy: "",
...
export function normalizeWorkbookPeptide(row) {
  ...
  const commonResearchGoals = parsePipeList(row.common_research_goals);
  const stackSynergies = parsePipeList(row.stack_synergies);
  ...
  const citationCount = parseCount(row.citation_count);
  ...
  return {
    ...
    mechanismSummary: row.mechanism_summary || sentenceFrom(row.mechanism_of_action) || "Mechanism summary pending verified researcher content.",
    primaryReceptor: row.primary_receptor || "",
    biologicalPathway: row.biological_pathway || "",
    clinicalTrialsCount: parseCount(row.clinical_trials_count),
    animalStudiesCount: parseCount(row.animal_studies_count),
    mechanisticStudiesCount: parseCount(row.mechanistic_studies_count),
    evidenceLevel: row.evidence_level || (references.length >= 3 ? "Moderate" : references.length >= 1 ? "Early" : "Limited"),
    firstDiscoveredYear: row.first_discovered_year || "",
    commonResearchGoals,
    stackSynergies,
    ...
    citationCount: citationCount ?? references.length,
    fdaStatus,
    developmentStage,
    references,
